...
export default {
  ...
  computed: {
    changeTotalNumberOfModels() {
      return this.fromNum1[0].value.toFixed(0);
    },
    changeTotalNumber() {
      return this.fromNum1[1].value.toFixed(0);
    },
    changeStandNumber() {
      return this.fromNum1[2].value.toFixed(0);
    }
  },
  created() {
    axios
      .get(this.$commonvalue.baseurl + 'dpService/getFjslsjFj', {
        // 还可以直接把参数拼接在url后边
        params: {
          title: '眼镜'
        }
